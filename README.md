...
- 📚 **字典管理** - 系统字段配置管理，支持 HR 和财务模块字段的字典化配置（树形结构）
- 🔐 **权限控制** - 细粒度的菜单和功能权限管理
...
- 🔍 **文件搜索工具** - 强大的文件内容搜索功能，支持多条件筛选
- 📝 **文本替换工具** - 智能文本批量替换，支持正则表达式和多规则管理
- 🔐 **编码解码工具** - Base64、URL、HTML实体、Unicode编码解码，支持哈希计算
- ⏰ **时间戳转换工具** - Unix时间戳与日期时间格式转换，支持时区转换和日期计算

### 核心特性

- 📚 **字典管理系统** - 支持系统字段的动态配置，HR 和财务模块的重要字段可通过字典管理随时更新维护
...
- 系统核心数据（6 个组织、7 个岗位、6 个用户）
- HR 模块示例数据（招聘、考勤、薪酬等）
- 财务模块示例数据（总账、固定资产、资金流水等）
- 字典数据（46 条，涵盖 HR 和财务模块字段）
...
2. **API 增强**
   ...
   - ✅ 菜单 API 支持 `tree=true` 参数返回树形结构
   - ✅ 字典 API 支持 `tree=true` 参数返回树形结构
   - ✅ 新增字典管理相关 API 接口
...
- 🎨 **图标配置** - 支持 Element Plus 图标库，灵活配置菜单图标
...
**症状**: 无法启动应用，报错"database is locked"
...
- 💡 [最佳实践](https://best-practices.example.com)

#### 2. 社区支持

- **GitHub Issues**: 报告Bug和功能请求
- **讨论区**: 技术交流和经验分享
- **Stack Overflow**: 编程问题求助
